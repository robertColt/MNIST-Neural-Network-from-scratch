...
from functions import loss_cross_entropy
...
class NeuralNetwork():
    ...
    def train(self, X, y, epochs, classes, learning_rate=1, hidden_units=64):
        ...
        for epoch in range(epochs):
            ...
            cost = loss_cross_entropy(y, self.layers[-1].a, train_examples)
            self.train_history.append(cost)
            print("epoch {}: cost {}".format(epoch, cost))

        print("final cost : ", cost)
        return self.layers, self.train_history
    ...
    def init_weights(self, hidden_units, input_size, classes):
        ...
        for i, layer in enumerate(self.layers):
            prev_units = input_size if i == 0 else self.layers[i-1].n_units
            layer.w = np.random.randn(layer.n_units, prev_units)
            layer.b = np.zeros((layer.n_units, 1))
    ...
    def feed_forward(self):
        ...
        for i,layer in enumerate(self.layers):
            X = self.X if i == 0 else self.layers[i-1].a
    ...
    def back_propagation(self, batch_size):
        ...
        for i, layer in enumerate(self.layers):
            if i == len(self.layers) - 1:
                layer.delta = layer.a - self.y
            else:
                next_layer = self.layers[i+1]
                layer.delta = np.multiply(np.matmul(next_layer.w.T, next_layer.delta),(layer.df(layer.z)))
    ...
    def update(self, learning_rate, beta=0.9):
        ...
        for i, layer in enumerate(self.layers):
            a = self.X if i == 0 else self.layers[i-1].a
